...
exports.DeleteUser = exports.ShowCurrentUser = exports.GetSingleUser = exports.GetProfile = exports.GetAllUsers = void 0;
const user_model_1 = require("../models/user.model");
// ✅ GET ALL USERS (Admin only)
const GetAllUsers = async (req, res) => {
    try {
        const users = await user_model_1.User.find({ role: "user" })
            .select("-password");
...
const ShowCurrentUser = async (req, res) => {
    try {
        ...
        const currentUser = await user_model_1.User.findById(req.user.id).select("-password");
        if (!currentUser) {
            return res.status(404).json({ message: "User not found" });
        }
