...
exports.registerUserSocketHandlers = void 0;
const user_model_1 = require("../models/user.model");
const workspace_model_1 = require("../models/workspace.model");
const activeUsers = [];
const registerUserSocketHandlers = (io, socket) => {
    // ===============================
    // ✅ USER ONLINE EVENT
    // ===============================
    socket.on("userOnline", async ({ userId, workspaceId }) => {
        ...
        if (!workspace)
            return console.warn("Workspace not found for userOnline");
        // Add socket to workspace room
        socket.join(workspaceId);
        // Track active user
        const isAlreadyActive = activeUsers.some((u) => u.userId === userId && u.workspaceId === workspaceId);
        if (!isAlreadyActive) {
    ...
    socket.on("userUpdated", async ({ userId, workspaceId, updatedFields }) => {
        ...
        const workspace = await workspace_model_1.Workspace.findById(workspaceId);
        if (!workspace)
            return console.warn("Workspace not found for userUpdated");
        // Optionally update in DB
        if (updatedFields) {
            await user_model_1.User.findByIdAndUpdate(userId, updatedFields);
        }
